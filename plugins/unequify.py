...
COMPLETED_BTN = InlineKeyboardMarkup(
    [
        [InlineKeyboardButton('⚡ Support', url='https://t.me/MoviesUpdate_chat')],
        [InlineKeyboardButton('📢 Updates', url='https://t.me/MoviesUpdate_07')]
    ]
)
...
async def unequify(client, message):
    ...
    print(f"temp.lock.get(user_id): {temp.lock.get(user_id)}")
    
    if temp.lock.get(user_id) and str(temp.lock.get(user_id)) == "True":
        return await message.reply("**please wait until previous task complete**")

    _bot = await db.get_bot(user_id)
    print(f"_bot: {_bot}, type(_bot): {type(_bot)}")
    ...
    print(f"target: {target}, type(target): {type(target)}, target.text: {target.text}")
    ...
    if target.text.startswith("/"):
        ...
        print(f"match: {match}, type(match): {type(match)}")
    ...
    elif hasattr(target, 'forward_from_chat') and target.forward_from_chat.type in ['channel', 'supergroup']:
    ...
    print(f"confirm: {confirm}, type(confirm): {type(confirm)}, confirm.text: {confirm.text}")
